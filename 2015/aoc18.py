...
def load(file):
  ...
  flock = []
  for idx, pos in enumerate(output.replace('\n','')):
    if pos == '#': flock.append(idx)
  return flock

def solve(data, epoch_max, epoch=0):
  global newborns
  flock = {k:[1,0] for k in data}
